...
from modules.o import SAIL, RoPE, Embedding, LMHead, RMSNorm, Linear
from modules import LMCoreMixin
from modules.conf import ModelConfig


class OctonionSelfAttention(nn.Module):
    """octonion multi-headed self-attention"""
    ...
    def __init__(self, config):
        ...
        # Die Projektionen für Q, K, V als HyperLinear Layer
    ...
    def forward(self, z1, z2, z3, z4):
        ...
        # projection
        q1, q2, q3, q4 = self.q_proj(z1, z2, z3, z4)
        k1, k2, k3, k4 = self.k_proj(z1, z2, z3, z4)
        v1, v2, v3, v4 = self.v_proj(z1, z2, z3, z4)
        ...
        # GyRoPE
        q1, q2, q3, q4 = self.rope(q1, q2, q3, q4)
        k1, k2, k3, k4 = self.rope(k1, k2, k3, k4)

        # octonion scalar product: Real(Q1 * conj(K1) + Q2 * conj(K2))
        ...
        # extract real part
        wei = (attn1 + attn2 + attn3 + attn4).real * (8 * self.head_size) ** -0.5
        ...
        # out projection
        out_z1, out_z2, out_z3, out_z4 = self.out_proj(out_z1, out_z2, out_z3, out_z4)
...
class Block(nn.Module):
    ...
    def __init__(self, config):
        ...
        super().__init__()
        self.sa = OctonionSelfAttention(config)
    ...
    def forward(self, z1, z2, z3, z4):
        # attention + residual
        sa_z1, sa_z2, sa_z3, sa_z4 = self.sa(z1, z2, z3, z4)
        norm_z1, norm_z2, norm_z3, norm_z4 = self.ln1(sa_z1, sa_z2, sa_z3, sa_z4)
        z1 = z1 + norm_z1
        z2 = z2 + norm_z2
        z3 = z3 + norm_z3
        z4 = z4 + norm_z4
        ...
        # ffwd + residual
        ff_z1, ff_z2, ff_z3, ff_z4 = self.ffwd(z1, z2, z3, z4)
        norm_ff_z1, norm_ff_z2, norm_ff_z3, norm_ff_z4 = self.ln2(
            ff_z1, ff_z2, ff_z3, ff_z4
        )
        z1 = z1 + norm_ff_z1
        z2 = z2 + norm_ff_z2
        z3 = z3 + norm_ff_z3
...
class KrakenFormer(nn.Module, LMCoreMixin):
    """release the dimensionally 8 armed transformer kraken 🐙"""
    ...
    def forward(self, idx, targets=None):
        B, T = idx.shape
        z1, z2, z3, z4 = self.token_embedding(idx)

        for block in self.blocks:
            z1, z2, z3, z4 = block(z1, z2, z3, z4)
        z1, z2, z3, z4 = self.ln_f(z1, z2, z3, z4)
        logits = self.lm_head(z1, z2, z3, z4)
